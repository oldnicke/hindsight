...
| `HINDSIGHT_API_FORGETTING_ENABLED` | Enable Ebbinghaus-style memory retrievability calculated from time since the memory was learned. This is separate from event recency and is disabled by default. | `false` |
| `HINDSIGHT_API_FORGETTING_MODE` | `observe` computes retrievability without changing ordering; `rank` applies a conservative multiplicative boost after reranking. | `observe` |
| `HINDSIGHT_API_FORGETTING_BASE_STABILITY_DAYS` | Stability `S` in `R = exp(-elapsed_days / S)`. Larger values make memories decay more slowly. | `30` |
